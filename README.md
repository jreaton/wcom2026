# WCOM Fall 2026 website — how to edit it
...
6. **Notes for maintainers** (technical details — Julie can ignore this).
...
*(Technical details for developers — not needed for editing content.)*

- **Stack:** [Jekyll](https://jekyllrb.com/) static site, deployed on GitHub Pages
  (builds automatically on push to `main`).
- **Ruby:** pinned to **Jekyll 3.9** in the `Gemfile` so it builds on the system
  Ruby (2.6) that ships with macOS — the same generation GitHub Pages runs, so
  local output matches the deployed site. If Ruby is upgraded to 3.0+, you can
  switch to the simpler `github-pages` gem.
- **Local commands:** `make site` (= `bundle install --path vendor/bundle` then
  `bundle exec jekyll serve --livereload`); `make clean` removes generated files.
- **Content model:** pages live in `_pages/` and `index.md`; the schedule and
  participant tables render from `_data/schedule.yml` and `_data/participants.yml`;
  layout/styling in `_layouts/`, `_includes/`, and `assets/css/style.css`.
- **Not published to the site:** `README.md`, `Requests.md`, and build artifacts
  are listed under `exclude:` in `_config.yml`.
